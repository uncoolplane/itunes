function Song(play, artist, song, collection, albumart, type, collectionprice) {
  this.play = play;
  this.artist= artist;
  this.song = song;
  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    this.getSongs = function(artist) {
      ...
      .success(function(response) {
  			console.log('service', response.results);
  			var isongs = response.results;
        songs = [];
        isongs.forEach(function(song) {
          //artist, song, collection, albumart, type, collectionprice
          songs.push(new Song(song.previewUrl, song.artistName, song.trackName, song.collectionName,
            song.artworkUrl60, song.primaryGenreName, song.collectionPrice));
        });
        deferrer.resolve(songs);
  		})
